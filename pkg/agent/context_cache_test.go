...
func setupWorkspace(t *testing.T, files map[string]string) string {
	t.Helper()
	tmpDir, err := os.MkdirTemp("", "nicaclaw-lite-test-*")
	if err != nil {
		t.Fatal(err)
	}
...
func TestSingleSystemMessage(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			sys := msgs[0].Content
			if !strings.Contains(sys, "nicaclaw-lite") {
	...
	for g := 0; g < goroutines; g++ {
		...
		go func(id int) {
			...
			for i := 0; i < iterations; i++ {
				...
				if !strings.Contains(result, "nicaclaw-lite") {
...
func BenchmarkBuildMessagesWithCache(b *testing.B) {
	tmpDir, _ := os.MkdirTemp("", "nicaclaw-lite-bench-*")
	defer os.RemoveAll(tmpDir)
