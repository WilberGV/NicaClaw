...
// GetModelConfigByIndex returns the n-th ModelConfig for the given model name.
func (c *Config) GetModelConfigByIndex(modelName string, index int) (*ModelConfig, error) {
	matches := c.findMatches(modelName)
	if len(matches) == 0 {
		return nil, fmt.Errorf("model %q not found in model_list", modelName)
	}
	if index < 0 || index >= len(matches) {
		return nil, fmt.Errorf("index %d out of bounds for model %q (count: %d)", index, modelName, len(matches))
	}
	return &matches[index], nil
}

// GetModelMatches returns all ModelConfig entries for a given model name.
func (c *Config) GetModelMatches(modelName string) []ModelConfig {
	return c.findMatches(modelName)
}
