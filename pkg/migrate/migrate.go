...
func Run(opts Options) (*Result, error) {
	...
	picoClawHome, err := resolveNicaclawliteHome(opts.NicaclawliteHome)
	if err != nil {
		return nil, err
	}
	...
	fmt.Println("Migrating from OpenClaw to nicaclaw-lite")
	fmt.Printf("  Source:      %s\n", openclawHome)
	...
	if !opts.WorkspaceOnly {
		...
		if err != nil {
			...
			actions = append(actions, Action{
				...
				Description: "convert OpenClaw config to nicaclaw-lite format",
			})
...
func executeConfigMigration(srcConfigPath, dstConfigPath, picoClawHome string) error {
	...
	if _, err := os.Stat(dstConfigPath); err == nil {
		...
		if err != nil {
			return fmt.Errorf("loading existing nicaclaw-lite config: %w", err)
		}
...
func resolveNicaclawliteHome(override string) (string, error) {
	if override != "" {
		return expandHome(override), nil
	}
	...
	return filepath.Join(home, ".nicaclaw-lite"), nil
}
