...
// It uses a matcher function to find all available instances of each model name
// to support multi-API/key fallback.
func ResolveCandidates(
	primary string,
	fallbacks []string,
	defaultProvider string,
	getMatches func(name string) int, // returns count of matches for a model name
) []FallbackCandidate {
	...
	addCandidate := func(raw string) {
		...
		// If this is a model name from model_list, it might have multiple instances
		count := getMatches(ref.Model)
		if count > 1 {
			// Add each instance as an indexed candidate
			for i := 0; i < count; i++ {
				indexedModel := fmt.Sprintf("%s[%d]", ref.Model, i)
				key := ModelKey(ref.Provider, indexedModel)
				if !seen[key] {
					seen[key] = true
					candidates = append(candidates, FallbackCandidate{
						Provider: ref.Provider + "/" + indexedModel, // Unique provider name for each key
						Model:    indexedModel,
					})
				}
			}
		...
		// Single instance or unknown model
